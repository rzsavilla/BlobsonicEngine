...
void GLSLProgram::printActiveUniforms() {
	...
	gl::GetProgramiv(handle, gl::ACTIVE_UNIFORM_MAX_LENGTH, &maxLen);
	gl::GetProgramiv(handle, gl::ACTIVE_UNIFORMS, &nUniforms);
...
void GLSLProgram::printActiveUniformBlocks() {
	...
	glGetProgramiv(handle, gl::ACTIVE_UNIFORM_BLOCK_MAX_NAME_LENGTH, &maxLength);
	glGetProgramiv(handle, gl::ACTIVE_UNIFORM_BLOCKS, &nBlocks);
	glGetProgramiv(handle, gl::ACTIVE_UNIFORM_MAX_LENGTH, &maxUniLen);
	...
	for (int i = 0; i < nBlocks; i++) {
		...
		glGetActiveUniformBlockiv(handle, i, gl::UNIFORM_BLOCK_BINDING, &binding);
		printf("Uniform block \"%s\" (%d):\n", name, binding);

		GLint nUnis;
		glGetActiveUniformBlockiv(handle, i, gl::UNIFORM_BLOCK_ACTIVE_UNIFORMS, &nUnis);
		GLint * unifIndexes = new GLint[nUnis];
		glGetActiveUniformBlockiv(handle, i, gl::UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES, unifIndexes);
...
void GLSLProgram::printActiveAttribs() {
	...
	glGetProgramiv(handle, gl::ACTIVE_ATTRIBUTE_MAX_LENGTH, &maxLength);
	glGetProgramiv(handle, gl::ACTIVE_ATTRIBUTES, &nAttribs);
