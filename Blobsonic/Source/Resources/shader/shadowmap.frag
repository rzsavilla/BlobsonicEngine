...
uniform Shadowmapping shadowmapping[SHADOW_MAP_COUNT];
...
vec3 calcShadowmap(ShadowMap light, vec4 shadowCoords, vec3 normal, vec3 fragmentPosition, vec3 viewDirection)
{
	//Calculate light direction to current vertex
	vec3 lightDirection = normalize(-light.direction);
